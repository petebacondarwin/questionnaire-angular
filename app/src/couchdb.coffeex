...
angular.module('CouchModule').service "CouchDBDocuments",
  ['$http', '$log',
  ($http, $log)->
    (dbUri)->
      # Save a document to the database
      this.save = (doc)->
        throw "Document has no _id" unless doc._id?

        $http.put("#{dbUri}/#{doc._id}", doc)
          .success (response)-> 
      ...
      this.get = (docId)->
        $http.get("#{dbUrl}/#{docId}")
          .success (response)-> response.data

      # Deletes a document from the database
      this.delete = (doc)->
        $http.delete("#{dbUrl}/#{doc._id}?rev=#{doc._rev}")
          .success (response)->
            doc._rev = response.data.rev
            doc._deleted = true
            response.data
